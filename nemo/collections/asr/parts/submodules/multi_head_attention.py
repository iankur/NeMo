...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_head, n_feat, dropout_rate, max_cache_len=0, group_size=1):
...
class RelPositionalEncoding(PositionalEncoding):
    ...
    def extend_pe(self, length, device):
        ...
        needed_size = 2 * length - (1 == self.group_size)
        if hasattr(self, 'pe') and self.pe.size(1) >= needed_size:
            return
        # positions would be from negative numbers to positive
        # positive positions would be used for left positions and negative for right positions
        positions = torch.arange(length - (1 == self.group_size), -length, -1, dtype=torch.float32, device=device).unsqueeze(1)
        self.create_pe(positions=positions)
        self.center_pos = torch.tensor(self.pe.size(1) // 2 + (1 == self.group_size), dtype=torch.int32, device=device)
    ...
    def forward(self, x, cache_len=0):
        ...
        if input_len % self.group_size:
            input_len += self.group_size - input_len % self.group_size
        start_pos = self.center_pos - input_len + (self.group_size if self.group_size > 1 else 0)
        end_pos = self.center_pos + input_len - int(1 == self.group_size)
